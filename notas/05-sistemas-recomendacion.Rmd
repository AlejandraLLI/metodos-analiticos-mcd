...
si $X_1,\ldots, X_k$ son mediciones de gusto por distintos items, y $\bar{X}$ es la estimación del nivel $N$, entonces $S_j=X_j-\bar{X}$ y se tiene que 
...
$\left( \text{es de orden} \tfrac{1}{k}\right)$, 
...
Para este conjunto de datos, al menos tenemos mejor score que el algoritmo utilizado en su momento por netflix. Sin embargo, nótese que estrictamente hablando no podemos comparar nuestros resultados con estos números,
...
No hemos resuelto el problema de películas o usarios con pocas evaluaciones. Como vimos arriba,
...
# Agrega la media de las películas a la base de datos de peliculas 
...
Si hay muchas evaluaciones, esto va a hacer que el ponderador sea muy cercano a uno y tenemos la misma medida que antes. Si hay pocas evaluaciones, entonces el ponderador va a ser cercano a cero y le doy más peso a la media general porque no confío en lo que estoy observando. 
...
ejemplos <- function(pelicula){
  ...
  out_sum <- datos_comp %>% # datos completos 
      group_by(peli_id) %>% #agrupa por película 
      summarise(similitud = sim_cos(calif_c, calif_c_1)) %>% #extrae similitud coseno
      left_join(medias_p_2) # agrega las medias ponderadas 
  
  # Ordena por distancia coseno y selecciona variables 
  out_sum %>% arrange(desc(similitud))  %>% select(nombre, similitud, num_calif_peli)
}
...
La idea es entonces encontrar pesos para películas $v$ y para usuarios $u$ de forma que
...
La idea ahora es que el gusto de una persona por una película se escribe como combinación de las dos dimensiones. Por ejemplo, para la persona $1$ y la película $1$, empezamos haciendo
...
- El renglón $k$ de $U$ son los valores en las dimensiones latentes para
el usuario $k$ (descriptores de usuarios).
- El renglón $l$ de $V$ son los valores en las dimensiones latentes para
la película $l$ (descriptores de películas)
...
y nótese que esta aproximación es en el sentido de las entradas de $X$ que **son observadas**. Sin embargo, $\widetilde{X}$ nos da predicciones para **todos los pares usuario-película**.
...
Si quisieramos tener una suma ponderada, entonces estimaríamos las calificaciones como $\tilde{X}=U\Lambda V'$ con $\Lambda$ matriz diagonal de pesos. 
...
de $p \times k$, donde $p$ es el número de películas tal que nos de una
...
es simple de calcular de forma analítica) para encontrar $U_1$. Una vez que encontramos $U_1$, la fijamos, e intentamos ahora resolver para $V_2$:
...
- Nótese que penalizamos el tamaño de los vectores $u_i$ y $v_j$ para evitar sobreajuste (como en regresión ridge).
...
Para cada evaluación $x_{i,j}$, la actualización de descenso está dada, en forma vectorial, por:
...
y
$$
v_j^{(n+1)} = v_j^{(n)} +\gamma e_{i,j} u_i^{(n)}
$$
...
\begin{equation}
L =  \sum_{(i,j) obs} (x_{ij} +\mu - \hat{a}_i - \hat{b}_j - \sum_{l=1}^k u_{i}^{(l)}v_{j}^{(l)})^2
\end{equation}

Y las derivadas adicionales están dadas por

$$\frac{\partial L_{i,j}}{\partial \hat{a}_i} = -2e_{i,j} 
$$

$$\frac{\partial L_{i,j}}{\partial \hat{b}_j} = -2e_{i,j} 
$$

y el resto de las derivadas (para otras $\hat{a}_s$ y $\hat{b}_t$) son iguales a $0$.
...
\begin{equation}
L =  \sum_{(i,j) obs} (x_{ij} + \mu - \hat{a}_i - \hat{b}_j - \sum_{l=1}^k u_{i,l}v_{j,l})^2 +
\lambda \left (  \sum_i ||u_i||^2 +  \sum_j ||v_j||^2 +  \sum_i \hat{a}_i^2 + \sum_j \hat{b}_j^2 \right )
(\#eq:regdescenso)
\end{equation}
...
$$
v_j^{(n+1)} = v_j^{(n)} + \gamma \left ( e_{i,j} u_i^{(n)} -  \frac{\lambda}{m_j} v_j^{(n)} \right )
$$
...
$$b_j^{(n+1)} = b_j^{(n)} + \gamma \left ( e_{i,j} - \frac{\lambda}{m_j} b_j^{(n)} \right )$$
...
Notar que el error de entrenamiento y de validación son muy cercanos. Cuando hay una divergencia muy grande entre estos (entrenamiento muy chico y validación muy grande) se tiene un indicador de sobre ajuste del modelo. Para controlar esto, tenemos las u's, v's y $\lambda$. Lo primero que podemos hacer es regularizar más o tomar menos factores. 
...
ggplot(filter(medias_pelis, num_calif_peli > 1000), aes(x = año, y = media_peli)) + 
...
- Sin embargo, estamos seguros de que niveles altos de interacción (oyó muchas veces una canción, etc.), es indicador de preferencia alta.
...
de $c_{ij}$ (confianza) actúan como pesos en el ajuste de
