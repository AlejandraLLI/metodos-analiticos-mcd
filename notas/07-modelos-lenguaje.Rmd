...
$$P(W|X)$$
es decir, la probabilidad del verdadero mensaje ($W$) dado que observé el mensaje transformado ($X$). 
...
que en principio es un máximo sobre todas las posibles frases del lenguaje. Es decir, eligo aquel mensaje $W$ que maximiza la verosimilitud. 

¿Cómo construimos esta probabilidad condicional? Siguiendo la regla de Bayes, tenemos que
para cada posible frase $W$,

$$P(W|X) = \frac{P(X|W)P(W)}{P(X)},$$

así, dado que $X$ es la constante de normalización, es equivalente a maximizar 

$$ W^* = \underset{W}{argmax} P(X|W)P(W).$$

Esta forma tiene dos partes importantes:


1. **Verosimilitud**: la probabilidad $P(X|W)$ de observar el mensaje transfromado $X$ dado que el verdadero mensaje es $W$. Este el el  **modelo del canal** (o **modelo de errores**), que nos dice cómo ocurren errores o transformaciones $X$ cuando se pretende comunicar el mensaje $W$. Por ejemplo $P(hila|hola)$ sería alta, pero $P(gato|hola)$ sería baja. 

2. **Inicial** o **previa**: La probabilidad $P(W)$ de observar el mensaje $W$ en el contexto (lenguaje) actual. Esto depende más del lenguaje que del canal, y le llamamos el **modelo del lenguaje**. Por ejemplo, $P(hola)$ es alta, pero $P(parangaricutirimucaro)$ es baja.
...
Supongamos que tenemos sólo cuatro frases en nuestro lenguaje:
...
# Dataframe del corpus con las probabiliades del lenguaje
...
```{r, echo=FALSE}
# Agrega las probabilidades condicionales (modelo del canal)
p <- data_frame(W = c("$W_1$","$W_2$", "$W_3$", "$W_4$"),
                `P(W)` = c(0.001, 0.001, 0.008, 0.000001),
...
```{r, echo=FALSE}
# Extrae la probabilidad condicional
p <- data_frame(W = c("$W_1$","$W_2$", "$W_3$", "$W_4$"),
...
p %>% knitr::kable(format = 'markdown')
```
de modo que escogeríamos la segunda frase "Estoy a diez minutos" (aquella con la máxima probabilidad condicional) como la interpretación de la frase
"Estoy a días minutos".

Una probabilidad condicional baja puede ocurrir porque es poco probable que se distorsione el mensaje $W$ en el mensaje $X$; o bien, porque el mensaje $W$ tenga muy baja probabildad de ocurrir. Nótese que $P(X|W_3)$ en particular es muy bajo,
...
donde $w_i$ son las palabras que contiene el texto $W$.
...
algo como ver una colección de ejemplos del lenguaje,
...
colección). Esto es, no funciona contar directamente todas las posibles frases y sus probabilidades.
...
Probablemente la dependencia de la palabra mil con la primer palabra es muy baja. Entonces, puedo irme no tan lejos para entender la probablidad de ocurrencia de una sola palabra. 
...
para cualquier conjunto de palabras $w_1,w_2,\ldots, w_m$. 
A estas $w_1,w_2,\ldots, w_m$ palabras que ocurren justo antes de $w$ les
...
$$P(w_n|w_1,w_2\cdots w_{n-1}) = P(w_n)$$
...
palabra sólo depende de palabra previa. De esta manera:
...
$P(w|z)$ para cada par de palabras del vocabulario $w,z$, lo cual
...
Este va a ser un mejor modelo, pero a cambio voy a necesitar más datos porque ahora tenque estimar $V^3$ posibilidades. 
...
que nos interesa.
¿Cómo podemos estimar las probabilidades $P(w|a)$ $P(w|a,b)$?

El enfoque más simple es estimación por máxima verosimilitud, que 
en este caso implica estimar con conteos de ocurrencia
...
Para el modelo de unigramas tendríamos que $P(w|z)=P(z)=\dfrac{N(z)}{N}$ o bien, $log P(w|z)=log N(z)-log N$. 

Para el modelo de trigramas, tendríamos que $P(w|y,z)=\dfrac{N(w,y,z)}{N(y,z)}$ o bien, $log P(w|y,z) = log N(w,y,z) - log N(y,z)$. 

#### Ejercicio {-}
Considera la colección de textos "un día muy soleado", 
"un día muy lluvioso", "un ejemplo muy simple de bigramas". Estima
las probabilidades $P(\text{muy})$, $P(\text{día} | \text{un})$, $P(\text{simple}| \text{muy})$ usando
conteos.

**Solución**
Posibles palabras: un, dia, muy, soleado, lluvioso, ejemplo, simple, de, bigramas. 

$P(\text{muy})=\dfrac{N(muy)}{N}}=\dfrac{3}{9}=\dfrac{1}{3}$ 

$P(\text{día}|\text{un})=\dfrac{N(un,dia)}{N(un)}=\dfrac{2}{3}$

$P(\text{simple}|muy)=\dfrac{N(muy,simple)}{N(muy)}=\dfrac{1}{3}$
...
$P(w)$ (una sola palabra), pues podemos poner como símbolo inicial
...
P(\langle s \rangle)P(el|\langle s \rangle)P(perro|el)P(corre|perro)P(\langle /s \rangle | corre)$$
...
$$
en donde queremos maximizar la probabilidad de que ocurra $w_a$ vs. la probabilidad de que ocurra alguna de las $v_j$.
...
```{r}
ant <- model[["grande"]] - model[["chico"]]
vector <-  model[["feo"]] + ant
model %>% closest_to(vector, n = 10) 
```
...
```{r, out.width = "800px", cache = TRUE,eval=FALSE}
