...
```{r, tidy=FALSE}
# Define la colección de documentos
...
```{r}
# Interpreta cada elemento del vector frases como un documento. 
vs <- VectorSource(frases)

# Asigna estructura de corpus
corpus <- VCorpus(vs)

# Extrae la matriz (rala) de términos-documentos. 
# weightBin hace referencia a pesos binarios (1 si el doc tiene la palabra, 0 si no la tiene)
# wordLengts hace referencia a la longitud aceptada de las palabras. 
# Palabras con número de caracteres fuera del intervalo son desechadas. 
...
# Imprime la matriz (densa) de términos-documentos.
td_mat
```
...
Primero exrtraemos todos los documentos que contienen **perro**
```{r}
# Extrae las columnas en las que el renglón "perro" tiene un uno 
...
De esos docuemtnos, extraemos los que tienen **gato**
```{r}
# Extrae las columnas en las que el renglón "gato" tiene un uno 
...
Por último, extraemos los documentos que **NO** tienen **casa**
```{r}
# Extrae las columnas en las que el renglón "casa" NO tiene un uno 
...
```{r}
# Imprime la información de la matriz sparse
td_sparse

# Estructura de la matriz sparse
# i:=palabras, j:=documentos, v:=1 (si doc j tiene la palabra i)
str(td_sparse)
```
...
para recuperación de documentos: el índice invertido. 
...
```{r}
# Creamos un data frame con los indices invertidos 
df_frases <- data_frame(id = 1:length(frases), frase = frases) %>% # id y documento
  unnest_tokens(palabras, frase) %>% # se extraen los tokens y el id al que corresponden
  group_by(id, palabras) %>% # se agrupan tokens por id y palabra
  summarise(term_frec = length(palabras)) %>% # se cuenta el no. de veces que aparece cada palabra en una misma frase 
  group_by(palabras) %>% # se agrupa por palabra 
  mutate(doc_frec = length(id)) # se cuenta el número de artículos en los que aparece


# Extrae una lista de dataframes para cada palabra con la frecuencia por documento y la frecuencia en documentos. 
ii_lista <- split(df_frases, df_frases$palabras) %>% # separa en dataframes individuales para cada palabra. 
  map(function(df){ select(ungroup(df), -palabras) %>% ungroup }) # desagrupa y elimina la columna palabras. 

# Impresión de la lista de dataframes 
ii_lista
```
...
```{r}
# Función para crear el índice invertido. 
crear_indice <- function(frases){
  
  # Asocia la funcion hash a un ambiente. 
  ...
  for(i in 1:length(frases)){
    ...
    # Convierte los tokens a factor 
    tokens_f <- factor(tokens)
    
    # Cuenta la frecuencia de cada palabra en la frase i. 
    tokens_conteo <- tabulate(tokens_f)
    
    # Extrae los nombres de cada factor (palabra)
    nombres <- levels(tokens_f)
    
    # Para cada palabra en la frase i...
    for(j in 1:length(tokens_conteo)){
      
      # Asigna la palabra 
      token <- nombres[j]
      
      # Al ambiente del token, le asigna el id del documento y la frecuencia de la palabra j en el docuemnto i. 
...
# Evalua la función crear_indice en las frases de prueba
ii_env <- crear_indice(frases)

# Extrae el índice invertido para la palabra el
ii_env$el
```
...
4. Creamos el índice invertido, que consiste de un diccionario indexado por los términos, y donde cada término apunta a los indentificadores de los documentos que contienen este término (el diccionario puede estar en memoria, y contener apuntadores a archivos de disco donde estan los doc_id)
```
...
```{r}
# Buscar documentos que contengan perro y gato. 
# Extrae los id de documentos que contienen perro (pull) y los id de documentos que contienen gato
# Luego encuentra la intersección 
perro_y_gato <- intersect(ii_lista[["perro"]] %>% pull(id), 
                ii_lista[["gato"]] %>% pull(id))

# Imprime las frases con perro y gato. 
frases[perro_y_gato]

```

Si queremos las frases que contengan perro **o** gato
Supongamos que buscamos perro **y** gato

```{r}
# Buscar documentos que contengan perro o gato. 
# Extrae los id de documentos que contienen perro (pull) y los id de documentos que contienen gato
# Luego encuentra la unión 
perro_o_gato <- union(ii_lista[["perro"]] %>% pull(id), 
                ii_lista[["gato"]] %>% pull(id))


# Imprime las frases con perro o gato. 
frases[perro_o_gato]
```
