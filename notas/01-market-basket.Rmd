...
- Por monotonicidad, si  $J$ es un conjunto de artículos más grande que $I$ (es decir $I\subset J$), entonces $n(J) \leq n(I)$: cualquier canasta que contiene a $J$ también contiene a $I$, y puede haber algunas canastas que contienen a $I$, pero que no contienen a $J$.
...
*lhs* es el antecedente, *rhs* es el consecuente, *support* es el soporte de la canasta $I\cup j$, *confidence* es la confianza de la regla de asociación, *lhs.support* es el soporte del antecedente, *lift* es el interés (ver más adelante) y *count* es el número de ocurrencias de la canasta $I\cup j$. 
...
- Valores altos de hiper-lift generan valores altos de lift, pero al revés no es necesariamente cierto. 
